<template>
	<base-drawer mode="right" :visible="visible" background-color="transparent" mask maskClosable @close="closeDrawer">
		<view class="edit-lable">
			...
			<view class="list" :style="[listH]" v-if="isStore">
			<!-- #endif -->
			<!-- #ifndef MP -->
...
export default {
	...
	computed: {
		listH(){
			let H = `calc(100% - (${this.getHeight.barTop+this.getHeight.barHeight+56}px + env(safe-area-inset-bottom)))`
			return{
				height: H
			}
		}
	...
	methods:{
		...
		productLabel(data,num,ids){
			...
			let funApi = '';
			if(this.storeNum){
				funApi = getProductLabel;
			}else{
				funApi = storeProductLabel;
			}
			funApi().then(res=>{
				res.data.map(el => {
