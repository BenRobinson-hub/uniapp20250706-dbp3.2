	...
	<view class="goods-list">
		...
		<view class="footer acea-row row-between-wrapper" v-if="administer">
			...
			<view class="acea-row row-middle">
				<view class="bnt acea-row row-center-wrapper" v-if="storeNum" @click="editLabels">添加标签</view>
		...
		<base-drawer mode="bottom" :visible="visible" background-color="transparent" mask maskClosable @close="closeDrawer">
			<view class="edit-list rd-t-40rpx">
				<view class="item" v-if="!storeNum && goodsInfo.pid==0" @tap="editInfo(0)">修改价格</view>
				<view class="item" v-if="!storeNum" @tap="editInfo(4)">修改库存</view>
				<view class="item" v-if="storeNum" @tap="editInfo(0)">修改价格/库存</view>
				<view class="item" @tap="editInfo(1)">商品分类</view>
				<view class="item" v-if="goodsInfo.pid==0" @tap="editInfo(2)">商品标签</view>
			</view>
		</base-drawer>
		<edit-price :visible='visiblePrice' :goodsInfo='goodsInfo' :storeNum='storeNum' @closeDrawer='priceCloseDrawer' @successChange='successChange'></edit-price>
		<edit-stock :visible='visibleStock' :goodsInfo='goodsInfo' @closeDrawer='stockCloseDrawer' @successChange='successChange'></edit-stock>
		<edit-lable ref="lable" :visible='visibleLable' :storeNum='storeNum' @closeDrawer='lableCloseDrawer' @successChange='successChange'></edit-lable>
		<classify ref="classify" :visible='visibleClass' :storeNum='storeNum' @closeDrawer='classCloseDrawer' @successChange='successChange'></classify>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			setShow(item,num,type){
				...
				let funApi = '';
				if(this.storeNum){
					funApi = productSetShow;
				}else{
					funApi = storeProductSetShow;
				}
			...
			productList(){
				...
				let funApi = '';
				if(this.storeNum){
					funApi = adminProductList;
				}else{
					funApi = storeProductList;
				}
				funApi({
					page: that.page,
					limit: that.limit,
				...
				switch (index) {
					case 0:
						...
						if(this.goodsInfo.spec_type){
							uni.navigateTo({
								url: '/pages/admin/goods/specs?id='+this.goodsInfo.id + '&storeNum='+this.storeNum
							})
						}else{
					...
					case 4:
					    ...
					    if(this.goodsInfo.spec_type){
					    	uni.navigateTo({
					    		url: '/pages/admin/goods/specsStock?id='+this.goodsInfo.id + '&storeNum='+this.storeNum
					    	})
					    }else{
			...
			openDrawer(item){
				...
				this.$refs.classify.category(item.id,item.cate_id,item.store_cate_id,item.relation_id);
			},
			closeDrawer(){
				this.visible = false
			},
