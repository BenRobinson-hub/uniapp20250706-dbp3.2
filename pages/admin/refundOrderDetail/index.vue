		...
		<view class="order-details pos-order-details">
			...
			<view v-for="(j, indexw) in orderInfo.split" :key="indexw" v-if="orderInfo.split && orderInfo.split.length">
				...
				<view class="pos-order-goods">
					<navigator :url="`/pages/admin/orderDetail/index?id=${j.order_id}&storeNum=${this.storeNum}`" hover-class="none" class="goods acea-row row-between-wrapper" v-for="(item, index) in j.cartInfo" :key="index">
			...
			<!-- 结束 -->
			<view class='wrapper' v-if="storeNum">
				<view class="store-name">{{orderInfo._status.refund_name}}</view>
				<view class="store-address">{{orderInfo._status.refund_address}}</view>
			</view>
			<view class='wrapper'>
			...
			<view class="wrapper">
				<view class="item acea-row row-between">
					...
					<view class="conter acea-row row-middle row-right">
						...
						<!-- #ifdef H5 -->
						<text class="copy copy-data" :data-clipboard-text="orderInfo.store_order_sn">复制</text>
						<!-- #endif -->
						<!-- #ifdef MP -->
	...
	export default {
		...
		methods: {
			...
			getIndex: function() {
				...
				let funApi = '';
				if(this.storeNum){
					funApi = getAdminRefundDetail;
				}else{
					funApi = getStoreRefundDetail;
				}
				funApi(that.order_id).then(res => {
					let num = 0;
