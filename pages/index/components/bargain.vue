<template>
	<view :style="[boxStyle]" v-if="bargList.length">
		<view>
			...
			<!-- 砍价列表 -->
			<!-- 单列 -->
			<view class="pt-32 pr-20 pb-32 pl-20" :style="[boxContentStyle]"
						...
						<view class="flex justify-between items-end">
							<view class="flex-col">
								...
								<text class="text-line fs-28 text--w111-999 pt-14 Regular"
									...
									:style="[otPriceColor]">¥{{item.product_price}}</text>
							</view>
				...
				<view v-for="(item,index) in bargList" :key="index"
					...
					<view class="flex justify-between items-end mt-10">
						<view class="flex-col">
							...
							v-if="checkboxInfo.includes(2)"></baseMoney>
							<text class="text-line fs-28 text--w111-999 pt-14 Regular"
								v-if="checkboxInfo.includes(3)" :style="[otPriceColor]">¥{{item.product_price}}</text>
						</view>
			...
			<view class="grid-column-3 grid-gap-18rpx pt-32 pr-20 pb-32 pl-20"
				...
				<view v-for="(item,index) in bargList" :key="index"
					...
					<view class="text-line fs-24 text--w111-999 Regular lh-32rpx"
						:style="[otPriceColor]" v-if="checkboxInfo.includes(3)">¥{{item.product_price}}</view>
				</view>
			</view>
			<!-- 滑动 -->
			<scroll-view scroll-x="true" show-scrollbar="false"
				...
				<view class="inline-block" :class="{ 'ml-20': index }" v-for="(item,index) in bargList" :key="index"
					@tap="bargDetail(item)">
					<easy-loadimage
					:image-src="item.image"
					width="224rpx"
