	...
	<view :style="[signWrapStyle]">
		<view class="signIn" :style="[signStyle]">
				...
				<view class="text">
					<view class="name-wrap acea-row row-middle">
						...
						<view class="num acea-row row-middle" :style="[numStyle]">
							<view class="inner" :class="{ opacity: !dataConfig.toneConfig.tabVal }">
								<image src="../../../static/images/sign-icon-01.png" mode="widthFix" class="icon"></image>
								+{{ sign_give_point }}
							</view>
						</view>
					</view>
					<view>连续签到{{nextContinuousSignRewardList.days||0}}天，额外活动{{nextContinuousSignRewardList.point||0}}积分</view>
				</view>
				<view class="button" :style="[buttonStyle]" @click="goUserSgin">立即签到</view>
			</view>
			<view v-else class="week-wrap acea-row row-middle">
				...
				<view class="button" :style="[buttonStyle]" @click="goUserSgin">签到</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getSign() {
				getSign().then(res => {
					this.nextContinuousSignRewardList = res.data.nextContinuousSignRewardList[0] || {};
					this.sign_give_point = res.data.sign_give_point;
