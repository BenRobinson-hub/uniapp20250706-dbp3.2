							...
							<view class="w-full flex-y-center mt-20">
								<view class="progress relative">
								    <view class="active" :style="{'width': item.percent +'%',background:progressBgColor}">
										<!-- <image src="@/static/img/lightning.png" class="lightning"></image> -->
									</view>
						...
						<view class="flex justify-between items-end">
							<view class="flex-col">
								...
								<text class="text-line fs-28 text--w111-999 pt-14 Regular"
									...
									:style="[otPriceColor]">¥{{item.product_price.toString().split('.')[1] ? item.product_price :  item.product_price + '.00'}}</text>
							</view>
					...
					<view class="flex justify-between items-end mt-10">
						<view class="flex-col">
							...
							<text class="text-line fs-28 text--w111-999 pt-14 Regular"
								...
								>¥{{item.product_price.toString().split('.')[1] ? item.product_price :  item.product_price + '.00'}}</text>
						</view>
			...
			<view class="grid-column-3 grid-gap-18rpx pt-32 pr-20 pb-32 pl-20" :style="[boxContentStyle]" v-if="goodStyleConfig == 2">
				<view v-for="(item,index) in spikeList" :key="index"
					...
					<view class="text-line fs-24 text--w111-999 Regular lh-32rpx mt-10"
						...
						>¥{{item.product_price.toString().split('.')[1] ? item.product_price :  item.product_price + '.00'}}</view>
				</view>
			</view>
			<!-- 滑动 -->
