    ...
    <view :style="[followWrapStyle]">
      <view :style="[followStyle]" class="follow acea-row row-between-wrapper">
        ...
        <view :style="[buttonStyle]" class="notes acea-row row-center-wrapper" @click="followTap">
          关注
          <!-- #ifdef MP-WEIXIN -->
          <official-account class="official-account" bindload="onLoadHandler" binderror="onErrorHandler"></official-account>
          <!-- #endif -->
        </view>
        <view class="iconfont icon-ic_close" @tap="hideFollow"></view>
      </view>
    </view>
...
export default {
  ...
  methods: {
	...
	onLoadHandler(e) {
		console.log('组件加载成功', e.detail)
	},
	onErrorHandler(e) {
		console.error('组件错误:', e.detail.errMsg)
	},
	hideFollow() {
		this.$emit('hideFollow');
	}
  }
}
</script>
