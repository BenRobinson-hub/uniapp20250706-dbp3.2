<!-- pages/index/components/flexibleWork.vue -->
<template>
 <!-- <view class="container" v-if="flexibleList.length && !isSortType">
    <view class="flexibleWorkList">
      <navigator
        v-for="(item, index) in flexibleList"
        :key="index"
        :url="'/pages/flexibleWork/job/detail?jobId=' + item.id"
        hover-class="none"
        class="item"
        :style="[flexibleItemStyle]"
      >
        <view v-if="dataConfig.styleConfig.tabVal != 0" class="image-wrap ">
          <image :src="item.image_input[0]" mode="widthFix" style="width: 100%;" />
        </view>
        <view class="text-wrap">
          <view class="title-wrap">
            <view class="title" :style="[titleStyle]">{{ item.title }}</view>
          </view>
          <view v-if="dataConfig.styleConfig.tabVal == 0" class="image-wrap">
            <image :src="item.image_input[0]" mode="widthFix" style="width: 100%;" />
          </view>
          <view class="time-wrap" :style="[numberStyle]">
            <view v-if="dataConfig.checkboxList.type.includes(0)" class="time" :style="[timeStyle]">
              {{ item.add_time }}
            </view>
            <view class="like-wrap" v-if="dataConfig.checkboxList.type.includes(1)">
              <text class="iconfont icon-ic_Eyes" :style="[iconEyesStyle]"></text>{{ item.visit }}
            </view>
          </view>
        </view>
      </navigator>
  ...
  <view v-show="!isSortType" :style="[articleWrapperStyle]">
  	...
  	}" v-if="flexibleList.length">
  		<navigator :url="'/pages/flexibleWork/job/detail?jobId=' + item.id" hover-class="none" v-for="(item,index) in flexibleList" :key="index" :style="[articleItemStyle]" class="item">
  			<view v-if="dataConfig.styleConfig.tabVal != 0" class="image-wrap">
  			...
  			<view class="text-wrap">
  				...
  				<view class="time-wrap" :style="[numberStyle]">
  					...
  					<view class="like-wrap time-wrap-item">
  						<view v-if="dataConfig.checkboxList.type.includes(1)" class="view" :class="!isLike?'on':''">
  							<text class="iconfont icon-ic_Eyes" :style="[iconEyesStyle]"></text>{{ item.visit }}
  						</view>
...
export default {
  ...
  computed: {
    isLike(){
    	...
    	if(this.dataConfig.styleConfig.tabVal){
    		like = !this.dataConfig.checkboxList.type.includes(1) && this.dataConfig.checkboxList.type.includes(2)
    	}else{
  ...
  methods: {
    getFlexibleList() {
      ...
      const activeValue = this.dataConfig.selectConfig?.activeValue || 0;
      getJobList({
        page: 1,
        limit: this.dataConfig.numConfig?.val >= limit ? limit : (this.dataConfig.numConfig?.val || limit),
        category: activeValue
      })
        .then(res => {
          this.flexibleList = res.data || [];
        })
        .catch(err => {
          console.error('获取灵活用工列表失败：', err);
          this.flexibleList = [];
        });
    }
  }
