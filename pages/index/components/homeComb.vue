		...
		<view class="my-main">
			<view class="mp-header" id="home" :style="[mpHeaderStyle]">
				...
				<view class="serch-box">
					<view class="serch-wrapper flex" :class="special?'on':''">
						<view class="map skeleton-rect" :style="[titleStyle]" v-if="searchBox == 0" @click="chooseLocation">
							<text class="iconfont icon-ic_location4" v-if="fixConfig == 1"></text>
							<text v-if="fixConfig == 0" class="fw-600">{{storeInfor.storeName.slice(0,7) || '选择门店'}}</text>
							<text v-else class="fw-600">{{addressInfo}}</text>
							<text class="iconfont icon-ic_downarrow" v-if="(fixConfig == 0 && belongIndex) || fixConfig == 1"></text>
						</view>
						<view class="title skeleton-rect" :style="[titleStyle]" v-if="searchBox == 1">{{titleConfig}}</view>
						<view class="logo skeleton-rect" v-if="logoShow">
			...
			<view v-if="!dataConfig.classConfig.tabVal" class="navTabBox tabNav">
				<view class="longTab" :style="{ width: `${mainWidth}px` }">
					<scroll-view scroll-x="true" scroll-with-animation :scroll-left="tabLeft" show-scrollbar="true">
						<view class="longItem" v-for="(item,index) in tabListConfig" :key="index" :class="{ click: index == tabClick }" :id="'id'+index" :data-index="index"
			...
			<view v-if="isCategory" class="category_count">
				...
				<view class="cate_count grid-column-4 grid-gap-16rpx mt-32">
					<view class="category_item" :style="[index===tabClick?classColor:'']" @click="changeTab(item,index)" v-for="(item,index) in tabListConfig" :key="index" :id="'ids'+index">
						{{item.text.val}}
					</view>
				</view>
			</view>
		</view>
		<view class="swiperBg" :style="{ paddingBottom: isMenu ? '30rpx' : '30rpx'}">
			<view class="swiper page_swiper" v-if="imgUrls.length">
				<swiper :autoplay="true" :circular="circular" :interval="intervals" :duration="duration" :current="swiperCur" :previous-margin="swiperMargin" :next-margin="swiperMargin"
					@change="swiperChange">
...
<script>
	...
	export default {
		...
		computed: {
			...
			logoUpImg() {
				let img = '';
				if(this.isScrolled && this.dataConfig.searchConfig.tabVal && this.dataConfig.logoUpConfig.url){
			...
			logoShow() {
				return this.searchBox == 2 && (this.dataConfig.logoConfig.url || (this.isScrolled && this.dataConfig.searchConfig.tabVal && this.dataConfig.logoUpConfig.url))
			},
			mpHeaderStyle() {
				let style = {};
