<template>
	...
	<view class="header" :style="[headerStyle]">
		<view class="serch-wrapper acea-row row-middle" :style="[serchWrapperStyle,txtPosition]">
			...
			<view class="map acea-row row-middle" :style="[txtStyle]" v-if="styleConfig==2 || (styleConfig==0 && styleTypeConfig == 1)" @click="chooseLocation">
				<text class="iconfont icon-ic_location51" v-if="fixConfig !=0"></text>
				<text class="fw-600 line1 inline-block flex-1" v-if="styleConfig===2 && fixConfig == 1">{{addressInfos}}</text>
				<text class="fw-600 line1 inline-block flex-1" v-if="styleConfig===2 && fixConfig == 0">{{storeInfor.storeName || '选择门店'}}</text>
				<text class="fw-600" v-if="styleConfig===0 && fixConfig == 1">{{addressInfo}}</text>
				<text class="fw-600" v-if="styleConfig===0 && fixConfig == 0">{{storeInfor.storeName.slice(0,7) || '选择门店'}}</text>
